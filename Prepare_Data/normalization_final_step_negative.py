...
def finalize(df):
    ...
    seq_cols = ['miRNA_sequence', 'site', 'full_mrna', 'mrna_bulge', 'mir_bulge', 'mrna_inter', 'mir_inter' ]
    df[seq_cols] = df[seq_cols].replace(to_replace='T', value='U', regex=True)
    logger.info("Add seed family")
    df["seed_family"] = df['miRNA_sequence'].apply(extract_seed_family)

    logger.info("Add valid/invalid flag")
    invalid_conditions = [pd.isna(df["miRNA_sequence"]),
                          pd.isna(df["site"]),
                          df["miRNA_sequence"].str.contains('X'),
                          df["miRNA_sequence"].str.contains('N'),
