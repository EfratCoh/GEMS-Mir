...
def add_features_to_graphs(g, sequence_dict, src_mirna_nodes, dst_mrna_nodes, feature_dim=5):
    ...
    for mRNA_node_id in range(dst_mrna_nodes):
        mRNA_position = mRNA_node_id
        nucleotide = fragment[mRNA_position] if mRNA_position < len(fragment) else "N"
        next_mRNA_position = mRNA_position + 1 if mRNA_position +1 < len(fragment) else -1
        mRNA_nucleotide_encoding = nucleotide_encoding_func(nucleotide)

        mRNA_node_feature = [mRNA_position + src_mirna_nodes, next_mRNA_position + src_mirna_nodes] + mRNA_nucleotide_encoding
