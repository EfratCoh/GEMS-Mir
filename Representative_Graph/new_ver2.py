...
def nucleotide_encoding_func(nucleotide):
    if nucleotide == 'A':
        return [1, 0, 0, 0]  # A -> [1, 0, 0, 0]
    elif nucleotide == 'U':
        return [0, 1, 0, 0]  # U -> [0, 1, 0, 0]
    elif nucleotide == 'C':
        return [0, 0, 1, 0]  # C -> [0, 0, 1, 0]
    elif nucleotide == 'G':
        return [0, 0, 0, 1]  # G -> [0, 0, 0, 1]
    elif nucleotide == 'N':
        return [0, 0, 0, 0]  # N -> [0, 0, 0, 0] (representing unknown nucleotide)
    else:
...
def add_features_to_graphs(g, sequence_dict, src_mirna_nodes, dst_mrna_nodes, feature_dim=5):
    ...
    # Pre-generate random features for efficiency
    total_nodes = src_mirna_nodes + dst_mrna_nodes
    random_features = np.random.uniform(200, 1000, (total_nodes, 83)).tolist()
    ...
    for miRNA_node_id in range(src_mirna_nodes):
        ...
        node_feature = [position, next_position] + nucleotide_encoding + random_features[miRNA_node_id]
    ...
    for mRNA_node_id in range(dst_mrna_nodes):
        ...
        mRNA_node_feature = [mRNA_position, next_mRNA_position+src_mirna_nodes] + mRNA_nucleotide_encoding + random_features[mRNA_node_id+src_mirna_nodes]

        # mRNA_node_feature = [mRNA_position] + mRNA_nucleotide_encoding
    ...
    for i in range(len(node_features)):
        node_features[i].append(in_degrees[i])  # Append in-degree as an additional feature
...
def graphClassification(in_dim, hidden_dim,n_classes, num_epoch, learning_rate):
    ...
    train_graphs, test_graphs, train_labels, test_labels = train_test_split(all_graphs, all_labels, test_size=0.3, random_state=42)
